...
### 4. External infra facts 與 platform control evidence defer policy

- `internal-testing` 可暫緩 external infra facts 與 platform control evidence，只要相關 plan / log 明確標記為 `deferred`，且不得把缺口寫成已解除、已驗證或已核准。
- 一旦任何 release、environment 或 gate 要被敘述成 `single-operator-production`，就必須先補齊對應 evidence；在 evidence 未完整前，不得宣稱 `production-ready`，也不得放行 production promote。
- 這裡的 evidence 至少應包含下列 examples，除非有更高要求的 authority 文檔另有補充：
	- 最近成功備份時間
	- 保留策略
	- RTO
	- RPO
	- restore rehearsal 日期 / 結果 / 證據
	- final sign-off
	- branch protection / required reviewers 的平台證據
- `cross-mode-governance` artifact 可以用來同時記錄 `internal-testing` 的 deferred gap 與 `single-operator-production` 的必補條件，但不得把兩種模式混寫成同一個已解除的結論。
...
- 後續 plan / log 若涉及 external infra facts 或 platform control evidence，應明寫是 `deferred`、`required before single-operator-production`，或已附證據，不得留下模糊 blocker wording。
