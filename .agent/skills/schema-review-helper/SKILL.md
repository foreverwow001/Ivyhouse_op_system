...
description: "Use when reviewing schema, master-data, shared-key, migration, or cross-module contract impact in a generic business-system project."
---

# Schema Review Helper

> 用途：降低 schema / master data / shared key / cross-module review 的執行摩擦。
> 這不是新的 gate 規格來源，也不負責決定是否一定要啟動 Domain Review、QA 或 migration gate；workflow hard contract 仍以 `.agent/workflows/AGENT_ENTRY.md` 為準。
...
1. 已讀 `project_rules.md` 或 `.agent/workflow_baseline_rules.md`（依工作區型態）
2. 已讀本輪 Plan 的下列區塊：
   - `File whitelist`
   - `MASTER DATA IMPACT`
...
- `.agent/skills/schema-review-helper/references/schema_checklist.md`
