...
| `schema-review-helper` | schema / master data / shared key / cross-module review 輔助 | 閱讀 `.agent/skills/schema-review-helper/SKILL.md` |
...
|------|---------------------------|------|----------|
| `plan-validator` | `python .agent/skills/plan-validator/scripts/plan_validator.py <plan_file_path>` | 驗證 Plan / work unit contract | 允許 |
| `targeted-unit-tests` | `python .agent/skills/test-runner/scripts/test_runner.py [test_path]` | 執行與目前 work unit 對應的 targeted tests | 允許 |
| `touched-file-lint` | `python .agent/skills/code-reviewer/scripts/code_reviewer.py <file_path|directory|diff>` | 對 touched files 做靜態檢查 | 允許 |
| `targeted-static-review` | `python .agent/skills/code-reviewer/scripts/code_reviewer.py <file_path|directory|diff>` | 額外靜態審查 token；V1 不預設啟用 | 保留 |
...
```bash
# 這是文件型 skill，直接閱讀內容後依序執行
cat .agent/skills/schema-review-helper/SKILL.md

# 詳細 checklist / compatibility rubric
cat .agent/skills/schema-review-helper/references/schema_checklist.md
```
