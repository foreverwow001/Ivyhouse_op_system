...
> Compatibility note (`Idx-029` Phase 4): 本工具文件僅保留 legacy PTY/fallback surface 的歷史說明，不再是正式 workflow authority。正式 workflow 入口、reviewer readiness 與 canonical contract 以 `.github/workflow-core/**` 與 `.workflow-core/**` 為準。
