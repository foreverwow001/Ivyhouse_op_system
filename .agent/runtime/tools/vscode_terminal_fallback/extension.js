...
function getConfig() {
  ...
  return {
    ...
    bootstrapTokenOnStartup: cfg.get("bootstrapTokenOnStartup", false),
    autoStartBridge: cfg.get("autoStartBridge", false),
    startupDelayMs: cfg.get("startupDelayMs", 1200),
...
async function bootstrapFallbackRuntimeOnStartup(context) {
  const cfg = getConfig();
  if (!cfg.bootstrapTokenOnStartup && !cfg.autoStartBridge) {
    return;
  }
  ...
  const delayMs = Math.max(0, Number(cfg.startupDelayMs) || 0);
  if (delayMs > 0) {
    await delay(delayMs);
  }
  ...
  try {
    if (cfg.bootstrapTokenOnStartup) {
      const tokenInfo = loadSendtextBridgeToken(root, true);
      result.tokenEnsured = Boolean(tokenInfo?.token);
      result.tokenPath = tokenInfo?.path || null;
      result.tokenSource = tokenInfo?.source || null;
    }

    if (cfg.autoStartBridge) {
      const bridgeInfo = startSendtextBridgeServer(context);
      result.bridgeRunning = Boolean(bridgeInfo?.running);
      if (!result.tokenPath) {
        result.tokenPath = bridgeInfo?.token?.path || null;
      }
      if (!result.tokenSource) {
        result.tokenSource = bridgeInfo?.token?.source || null;
      }
    }
  } catch (error) {
    result.status = "fail";
    result.error = String(error || "startup_bootstrap_failed");
    logLine(`[startup] fallback bootstrap failed: ${result.error}`);
  }

  appendDebugEvent(cfg, result);
}
...
function activate(context) {
  ...
  void bootstrapFallbackRuntimeOnStartup(context);
}
