...
def expand_mutable_aliases(path: str) -> list[str]:
...
def get_state_patterns(manifest: dict[str, Any]) -> list[str]:
        ...
        if isinstance(item, dict):
            for target in split_compound_targets(str(item.get("recommended_target", ""))):
                if any(path_matches_pattern(target, managed_pattern) for managed_pattern in managed_patterns):
                    continue
                patterns.extend(expand_mutable_aliases(target))

    skill_ownership = manifest.get("skill_ownership", {})
    local_target = skill_ownership.get("local_install_target")
    if local_target:
        for target in split_compound_targets(str(local_target)):
            patterns.extend(expand_mutable_aliases(target))
    ...
    for item in manifest.get("excluded_paths", []):
        ...
        if category == "environment-local":
            patterns.extend(expand_mutable_aliases(path))
        if path.startswith(".service/"):
            patterns.append(path)
        if path.startswith((".workflow-core/state/", ".workflow-core/config/")):
            patterns.extend(expand_mutable_aliases(path))
