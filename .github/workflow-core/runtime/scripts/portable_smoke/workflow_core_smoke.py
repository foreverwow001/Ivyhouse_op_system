...
def detect_workflow_entry(repo_root: Path) -> Path | None:
    runtime_surface = detect_runtime_surface()
    if runtime_surface == "canonical":
        candidate = repo_root / ".github/workflow-core/AGENT_ENTRY.md"
...
def run_portable_smoke(repo_root: Path, manifest_path: Path) -> dict:
    ...
    workflow_entry = detect_workflow_entry(repo_root)
    ...
    checks = {
        ...
        "agent_entry_present": workflow_entry is not None,
    ...
    return {
        ...
        "agent_entry_path": str(workflow_entry.resolve()) if workflow_entry else None,
        "checks": checks,
