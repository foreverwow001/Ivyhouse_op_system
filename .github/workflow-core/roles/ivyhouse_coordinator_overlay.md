...
4. `workflow_preflight_reviewer_cli.py --json`
...
10. Close 前執行 `workflow_preflight_reviewer_cli.py --json`
...
```bash
python .github/workflow-core/runtime/scripts/vscode/workflow_preflight_reviewer_cli.py --repo-root . --json
```

結果不是 `status=ready`，則不得把本輪宣告為正式執行完成。
