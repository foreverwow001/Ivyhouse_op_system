...
def build_filename(payload: Dict[str, Any], notes_dir: Path) -> str:
    ...
    role = slugify(str(payload.get("recorded_by_role", "unknown")), max_length=24, empty_slug="event")
    event_class = slugify(str(payload.get("event_class", "event")), max_length=32, empty_slug="event")
    signature = slugify(str(payload.get("symptom_signature", "event")), max_length=72, empty_slug="event")
