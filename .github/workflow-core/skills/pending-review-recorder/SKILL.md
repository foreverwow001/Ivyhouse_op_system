...
description: "用於判斷工作事件是否應記錄到 pending-review-notes、與既有 note 做去重比對，並為 engineer、QA 或 security reviewer workflow 建立或更新結構化 triage note。"
---
...
把工作過程中值得保留、但尚未形成正式結論的事件，穩定地寫入 `pending-review-notes`，並避免 agent 因重複記錄而產生大量相似筆記。

這個 skill 的定位是：

- triage intake，不是正式知識發佈器
- dedupe / update 決策器，不是 raw log dump 工具
- 角色共用能力，可被 engineer、QA、security reviewer 共用
...
- canonical script: `.github/workflow-core/skills/pending-review-recorder/scripts/pending_review_recorder.py`
- role overlays:
...
```bash
python .github/workflow-core/skills/pending-review-recorder/scripts/pending_review_recorder.py \
...
1. 已讀 active rule source
  - template repo 維護：`.github/workflow-core/workflow_baseline_rules.md`
   - downstream project repo：`project_rules.md`
2. 已讀對應角色 base role
  - engineer：`.github/workflow-core/roles/engineer.md`
  - QA：`.github/workflow-core/roles/qa.md`
  - security reviewer：`.github/workflow-core/roles/security.md`
