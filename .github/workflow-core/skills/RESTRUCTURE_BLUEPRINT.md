...
> 建立日期：2026-03-14
> 狀態：Phase 5 package-only cleanup 已完成
> 用途：把 `.github/workflow-core/skills/` 從「平鋪工具箱 + 過渡期 helper 文件」整理成可對齊 Claude / Antigravity skill 結構的目錄藍圖。
...
目前 `.github/workflow-core/skills/` 已完成 package-only 收斂，核心結構包括：
...
- shared helper / metadata：`_shared/__init__.py`、`.workflow-core/state/skills/skill_manifest.json`、`.workflow-core/config/skills/skill_whitelist.json`、`.workflow-core/state/skills/INDEX.local.md`、`schemas/`
...
> 備註：`_shared/` 現在只保留 path resolver 與 shared helper code；runtime state 與 project-local policy 已拆到 `.workflow-core/state/skills/` 與 `.workflow-core/config/skills/`。external/local skills 也已分流到 `.workflow-core/skills_local/`，不再與 builtin core packages 共用 `.github/workflow-core/skills/`。
...
| 現有檔案 | 建議分類 | 說明 |
|----------|----------|------|
| `.workflow-core/state/skills/skill_manifest.json` | shared metadata | canonical 技能清單索引 |
| `.workflow-core/config/skills/skill_whitelist.json` | governance data | canonical 外部下載來源白名單 |
...
- [x] manifest canonical write path 改到 `.workflow-core/state/skills/skill_manifest.json`
- [x] whitelist canonical write path 改到 `.workflow-core/config/skills/skill_whitelist.json`
- [x] audit log canonical write path 改到 `.workflow-core/state/skills/audit.log`
...
- [x] `.github/workflow-core/skills/INDEX.md` 改為 builtin-only core catalog
- [x] local/generated index 改寫到 `.workflow-core/state/skills/INDEX.local.md`
- [x] external/local skill install destination 改到 `.workflow-core/skills_local/**`
...
- root shim 已移除，所有調用統一使用 `python .github/workflow-core/skills/<package>/scripts/<tool>.py`
- `schemas/` 不搬進 `_shared/`，避免一次打破所有既有 validator/path 引用
- core `INDEX.md` 維持 stable builtin catalog；local additions 一律寫到 `.workflow-core/state/skills/INDEX.local.md`
- 外部技能下載後改以 `.workflow-core/skills_local/` package 目錄落地，避免重新長回 root `.py` 結構，也避免污染 builtin core package tree
