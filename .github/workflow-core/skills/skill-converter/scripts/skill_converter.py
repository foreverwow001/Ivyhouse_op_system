# -*- coding: utf-8 -*-
"""
skills/skill-converter/scripts/skill_converter.py
=====================================
用途：技能轉換流水線模組
...
def validate_approval(user_confirmed: bool) -> Dict[str, Any]:
    if not user_confirmed:
        return {
            ...
            "action_required": "請使用：python .github/workflow-core/skills/github-explorer/scripts/github_explorer.py download <repo> <file> --confirm",
        }
...
def resolve_package_root(skill_name: str, path: Path) -> Tuple[Path, str]:
    ...
    if path.is_relative_to(local_package_root) or (local_package_root.exists() and not path.is_relative_to(core_package_root)):
        return local_package_root, f".workflow-core/skills_local/{package_dir}"
    if path.is_relative_to(core_package_root) or core_package_root.exists():
        return core_package_root, f".github/workflow-core/skills/{package_dir}"
    ...
    if inferred_root.is_relative_to(LOCAL_SKILLS_DIR):
        return inferred_root, f".workflow-core/skills_local/{inferred_root.relative_to(LOCAL_SKILLS_DIR).as_posix()}"
    if inferred_root.is_relative_to(SKILLS_DIR):
        return inferred_root, f".github/workflow-core/skills/{inferred_root.relative_to(SKILLS_DIR).as_posix()}"
