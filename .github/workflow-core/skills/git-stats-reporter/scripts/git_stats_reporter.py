...
使用方式：
    python .github/workflow-core/skills/git-stats-reporter/scripts/git_stats_reporter.py <diff_file_path>
"""
...
def compute_triggers(affected_paths: List[str], total_lines_changed: int) -> Dict[str, bool]:
    maintainability_gate = total_lines_changed > MAINTAINABILITY_THRESHOLD
    ui_ux_gate = False

    for path in affected_paths:
        if any(path.startswith(prefix) for prefix in MAINTAINABILITY_CORE_PATHS if prefix.endswith("/")):
            maintainability_gate = True
        if path == "config.py":
            maintainability_gate = True

        if path.startswith(UI_UX_PATH_PREFIXES) or path in UI_UX_ENTRY_FILES or path.endswith(UI_UX_NAME_SUFFIXES):
            ui_ux_gate = True
...
def generate_report(diff_content: str) -> Dict[str, Any]:
    ...
    affected_paths = sorted({row.path for row in rows})
    total_lines_added = sum(row.added for row in rows)
    total_lines_deleted = sum(row.deleted for row in rows)
...
def error_result(summary: str, suggestion: str | None = None) -> Dict[str, Any]:
    result: Dict[str, Any] = {
        ...
        "summary": summary,
        "usage": "python .github/workflow-core/skills/git-stats-reporter/scripts/git_stats_reporter.py <diff_file_path>",
    }
    if suggestion:
...
def main(argv: List[str] | None = None) -> int:
    ...
    report = validate_output_schema(generate_report(diff_content), "git_stats_reporter")
