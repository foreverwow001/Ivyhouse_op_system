...
def is_builtin_record(record: Dict[str, Any]) -> bool:
    ...
    if isinstance(path, str) and path.startswith(".github/workflow-core/skills/") and not is_external_record(record):
        return True
    return False
...
def main(argv: List[str] | None = None) -> int:
    ...
    if "--sync" not in args and "--check" not in args:
        result = {
            ...
            "usage": "python .github/workflow-core/skills/manifest-updater/scripts/manifest_updater.py [--sync|--check]",
    ...
    try:
        ...
        result = {
            ...
            "usage": "python .github/workflow-core/skills/manifest-updater/scripts/manifest_updater.py [--sync|--check]",
            "suggestion": "請確認 skill_manifest.json 格式正確，且 .github/workflow-core/skills 目錄可讀取。",
        }
        print(json.dumps(validate_output_schema(result, "manifest_updater"), ensure_ascii=False, indent=2))
        return 2
