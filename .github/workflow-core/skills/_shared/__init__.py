...
CURRENT_SKILLS_DIR = SHARED_DIR.parent
...
def _canonical_static_relpath(*parts: str) -> str:
    return ".github/workflow-core/" + "/".join(part.strip("/") for part in parts if part)


def _mutable_relpath(*parts: str) -> str:
    return ".workflow-core/" + "/".join(part.strip("/") for part in parts if part)
...
BOUNDED_WORK_UNIT_ALLOWED_CHECKS = {
    "plan-validator": {
        ...
        "command": "python .github/workflow-core/skills/plan-validator/scripts/plan_validator.py <plan_file_path>",
    ...
    "targeted-unit-tests": {
        ...
        "command": "python .github/workflow-core/skills/test-runner/scripts/test_runner.py [test_path]",
        "default_status": "allowed",
        "notes": "Run only the targeted tests that map to the current work unit.",
    },
    "touched-file-lint": {
        "skill_name": "code_reviewer",
        "command": "python .github/workflow-core/skills/code-reviewer/scripts/code_reviewer.py <file_path|directory|diff>",
        "default_status": "allowed",
...
def read_manifest() -> Dict[str, Any]:
    ...
    return read_json_file(CANONICAL_MANIFEST_PATH, default)
...
def read_whitelist() -> Dict[str, Any]:
    ...
    return read_json_file(CANONICAL_WHITELIST_PATH, default)
...
def get_public_schema_path(skill_name: str) -> str | None:
    ...
    if schema_file is None:
        return None
    return _canonical_static_relpath("skills", "schemas", schema_file.name)
...
def get_package_metadata(skill_name: str) -> Dict[str, str] | None:
    ...
    return {
        "path": _canonical_static_relpath("skills", package_dir, script_rel),
        "package_path": _canonical_static_relpath("skills", package_dir, entry_doc),
        "script_path": _canonical_static_relpath("skills", package_dir, script_rel),
    }
