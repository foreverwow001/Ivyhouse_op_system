...
6. 所有 user-facing formal gate 一律以 VS Code `#askQuestions` 收集，且應盡量 batch 成單次或最少次數的 askQuestions 流程。
7. 若 `#askQuestions` surface 缺失或失效，視為 workflow environment blocker；不得改用一般聊天要求使用者重貼 prompt 或逐題補填 formal gate 決策。
