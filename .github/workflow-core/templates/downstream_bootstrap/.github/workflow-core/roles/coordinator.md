...
1. 按 `dev.md` 的 stage 順序推進 workflow，並在每個 gate 使用 askQuestions-first 與 user 明確確認；formal gate 決策不得改用一般聊天收集。
...
- 若 `#askQuestions` surface 缺失或失效，視為 workflow environment blocker；只能回報 blocker 與建議處置，不能把 gate 改成一般聊天逐題確認。
