...
## 遵守規範
請遵守 repo 規範：
- [.github/workflow-core/roles/coordinator.md](../roles/coordinator.md)
- [.github/workflow-core/workflows/dev.md](../workflows/dev.md)
- [.github/workflow-core/workflow_baseline_rules.md](../workflow_baseline_rules.md)（維護 template repo 時）
- [project_rules.md](../../project_rules.md)（下游/新專案時）

active workflow artifact 路徑固定為：`doc/implementation_plan_index.md`、`doc/plans/Idx-XXX_plan.md`、`doc/logs/Idx-XXX_log.md`。
...
- 管理 Copilot Chat engineer 執行面與 one-shot reviewer session 邊界
- 監控 targeted checks、reviewer output 與 Log 回填
...
- 詢問並記錄 `executor_tool`（`copilot-chat-agent`），寫入 `EXECUTION_BLOCK`
- 若 `security_review_required=true`，也必須先詢問並記錄 `security_reviewer_tool`
- 注入 / reviewer 任務時，必須要求固定 handoff package 與 reviewer output 結構
...
### 標準流程
以 Copilot Chat engineer 回傳的 handoff package、targeted checks 與 reviewer output 做完成判定。

### 備援
若 reviewer CLI 不可用，請使用者修復 `copilot` CLI 或改由人工 reviewer 提供結果；不再切換 PTY / fallback runtime。
